...
public class MoveRule extends Ruleset
{
	...
	public byte[][] apply(PatternWrapper cells)
	{
		...
		for (int x = -1; x < cells.getW()+1; x++)
			for (int y = -1; y < cells.getH()+1; y++)
			{
				...
				if (counter == 4 && Engine.getByte(cells.pattern, x, y)!=0)
					returnCells[x+1][y+1] = 1;
				if (counter == 5 && Engine.getByte(cells.pattern, x, y)!=0)
					returnCells[x+1][y+1] = 1;
				if (counter == 3 && Engine.getByte(cells.pattern, x, y)==0)
					returnCells[x+1][y+1] = 1;
				if (counter == 6 && Engine.getByte(cells.pattern, x, y)==0)
					returnCells[x+1][y+1] = 1;
				if (counter == 8 && Engine.getByte(cells.pattern, x, y)==0)
					returnCells[x+1][y+1] = 1;
			}
		return returnCells;
	}
