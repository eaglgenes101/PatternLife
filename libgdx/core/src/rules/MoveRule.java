	...
	public boolean[][] apply(PatternWrapper cells)
	{
		...
		for (int x = -1; x < cells.getW()+1; x++)
			for (int y = -1; y < cells.getH()+1; y++)
			{
				...
				if (counter == 4 && Engine.isActive(cells.pattern, x, y))
					returnCells[x+1][y+1] = true;
				if (counter == 5 && Engine.isActive(cells.pattern, x, y))
					returnCells[x+1][y+1] = true;
				if (counter == 3 && !Engine.isActive(cells.pattern, x, y))
					returnCells[x+1][y+1] = true;
				if (counter == 6 && !Engine.isActive(cells.pattern, x, y))
					returnCells[x+1][y+1] = true;
				if (counter == 8 && !Engine.isActive(cells.pattern, x, y))
					returnCells[x+1][y+1] = true;
			}
		return returnCells;
	}
